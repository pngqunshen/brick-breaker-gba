...
void initialiseLevelTwo(void) 
{
	...
	drawSprite(BALL, BALL_IND, ball_x, ball_y);
	// brick
	int level2_layout[] = {2,2,2,2,2,2,2,2,2,2,2,2,2,2,2,2,2,2,2,2,2,2,2,2,2,2,2,};
	for (i = 0; i < BRICK_MAX_NUM; i++) {
		bricks[i][0] = -1;
		bricks[i][1] = -1;
		brick_health[i] = BRICK_MAX_HEALTH;
	...
	for (i = 0; i < 45; i++) {
		...
		bricks[i][0] = xb+8;
		bricks[i][1] = yb+4;
		brick_health[i] = level2_layout[i];
		if (level2_layout[i] == 1){
			drawSprite(NUMBER_ONE, BRICKS_IND + i, xb, yb);
		}
		else if (level2_layout[i] == 2){
			drawSprite(NUMBER_TWO, BRICKS_IND + i, xb, yb);
		}
		else if (level2_layout[i] == 3){
			drawSprite(NUMBER_THREE, BRICKS_IND + i, xb, yb);
		}
		else {
			drawSprite(NUMBER_FOUR, BRICKS_IND + i, xb, yb);
